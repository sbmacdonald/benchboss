...
exit
	...
	if [[ $(trim $CGI_newemail) != $(trim $CGI_email) ]] ; then
	  ...
	  sendRegistrationEmail $(trim $CGI_newemail)
          ...
          sed -i "s|^$(trim $CGI_email)~\(.*\)\(~[u|c]$\)|$(trim $CGI_newemail)~$CGI_fname~$CGI_lname~$type~$pos\2|" ${ROSTER}
	fi
      else
    ...
    add)
      ...
      if [[ $(trim $CGI_newemail) != "" &&  $(trim $CGI_fname) != "" && $(trim $CGI_lname) != "" ]] ; then
        ...
        if [ `grep -c "^$(trim $CGI_newemail)" ${ROSTER}` -eq 0 ] ; then 
          chmod 755 ${ROSTER}
          echo "$(trim $CGI_newemail)~$CGI_fname~$CGI_lname~$type~$pos~u" >> ${ROSTER}
          #sed -i 's|skater|s|g' ${ROSTER}
          #sed -i 's|spare|s|g' ${ROSTER}
          #sed -i 's|regular|r|g' ${ROSTER}
          chmod 777 ${ROSTER}
          logInfo "Add($REMOTE_USER): $(trim $CGI_newemail)~$CGI_fname~$CGI_lname~$type~$pos~u" >> $rosterLogFile
          #if [[ "$type" == "r" ]] ; then
